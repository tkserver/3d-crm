  ...
  const fetchOrders = useCallback(() => {
    ...
    fetch(url)
      ...
      .then(data => {
        ...
        const fetchPromises = data.map(order =>
          order.id
            ? fetch(`/api/orders/${order.id}/items`)
                .then(r => r.json())
                .then(items => ({ ...order, items: Array.isArray(items) ? items : [] }))
                .catch(() => ({ ...order, items: [] }))
              ...
              {orders.map(order => {
                ...
                return (
                  <tr key={order.id} style={{ cursor: 'pointer' }} onClick={() => setSelectedOrder(order)}>
                    ...
                    <td style={styles.td}>
                      <button onClick={(e) => { e.stopPropagation(); setEditingOrder(order); }} style={{ ...styles.actionButton, background: '#3b82f6', marginRight: '0.5rem' }}>Edit</button>
...
const styles = {
  ...
  actionButton: { color: 'white', border: 'none', padding: '0.25rem 0.75rem', borderRadius: '4px', cursor: 'pointer' },
  ...
  modalContent: { background: 'white', padding: '2rem', borderRadius: '8px', minWidth: '400px', maxWidth: '90vw', maxHeight: '90vh', overflow: 'auto' }
};
